...
# For own experiment you could try with different methods 
nmds_rootAndSoil <- phyloseq::ordinate(physeq_filtered_CSS, # use 'ordinate' function from package phyloseq as many other packages have the same name of the function
...
nmds_rootAndSoil$points #This is a multi-layered object, check other options after the $
...
metadata <- as(sample_data(physeq_filtered_CSS), "data.frame") #getting metadata from phyloseq and saving it as a different object 
...
NMDS_Plant_species <- NMDS_listing(pslist_sp_CSS)
...
NMDS_Root_soil[2][[1]][1] #only gives you the root plot
NMDS_Root_soil[2][[1]][2] #only gives you the soil plot

#saving as objects
NMDS_AT_BO<-NMDS_listing(pslist_sp_CSS)
NMDS_4_slice<-NMDS_listing(ps_list_CSS)

NMDS_4_slice[1][[1]]$Arabidopsis_thaliana.Root #gives you only the NMDS output for A.thalia root

map(NMDS_4_slice[1][[1]], "stress") #with this you get the stress value for all elements in the list from the NMDS



rm(output, CSS_matrix) #check later what is not needed 
```


# 2.2 - Beta Dispersion

Beta dispersion indicates variance in the multidimensional space. It essentially measures the spread of your data points in the ordination, according your different treatments. As you might expect, statistical tests will have a hard time comparing a treatment with high beta dispersion (wide, sparse datapoint cloud) against a treatment with low beta dispersion (clustered, compact datapoint cloud), just like it is difficult to compare large error bars to small error bars.  You can test for homogeneiety of this variance, similarly to a Levene test of homogeneity before an anova. 
...
beta_disp_plotAndTest<-function(phyloseq_list, group_to_analyse){
  ...
  betadisper(d = phyloseq::distance(t(otu_table(x)), method="bray"), 
             group = sample_data(x)[[group_to_analyse]])) #selects only column "group""
...
dip_result3$Arabidopsis_thaliana.Root$eig #to get eigenvalues for each PCoA component 


rm(dip_resi;t)
# that was much better! note that we calculate the homogeneity within the function, access the P value of the test, and then place it on the plot
#It's common to take a lot of time to handle an apparently simple problem when writing these functions. Pedro took ~3h to make the "group" in the function be correctly evaluated and executed...

#CONCLUSION:  all beta dispersions are similar. we can safely trust in the permanova results
```
...
# now let's calculate the permanova (*check whether you can get AIC values)
...
permanova_with_blocks<-function(phyloseq_list, rhs_model){
  ...
  lapply(phyloseq_list, function (x){
    ...
    form<- as.formula(paste("lhs_data~", paste(rhs_model_in))) # getting the formula properly evaluated as custom string is tricky
       output<-adonis2(form, 
              data = as(sample_data(x),"data.frame"), # changing with as.data.frame is insufficient
...
#adonis2 takes different parameters for permutation than adonis
