...
imp_class_l<-as.character(tax_table(ps_important_taxa)[,"Class"]) # gets a char vector  of families shown as relevant
imp_class_l<-names(which(table(imp_class_l)>2))%>% # get names of taxa that occu ate least 3 times
...
imp_genus_l<-names(which(table(imp_genus_l)>2))%>%
  na.omit()%>% 
  unique()%>% 
  as.list(c()) #PEDRO REMOVE UNCULTURED TAXA FROM HERE
...
fisher_result<-mapply(function (target_in_important_n,target_in_all_n)
  fisher.test(matrix(c(target_in_important_n, 
                       ...
                       target_in_all_n, 
                       all_taxa_in_all_n - target_in_all_n),
                     ncol=2)), 
