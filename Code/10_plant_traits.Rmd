...
ldry_weight <- Bole$ldry_weight

colnames(Bole)

trait_name <-names(ldry_weight)
 x <- Bole
...
df.cohensD <- rbind(a,b,c)%>% 
              ...
              mutate(eff_size=
                       case_when(
                         ...
                         magnitude == 3 ~ "medium",
                         magnitude == 4 ~ "large"))%>%
             relocate(trait, treatment, eff_size)
...
trait <- "ldry_weight"
variable<- noquote("ldry_weight")

cohensD_per_trait <- function(df, trait){
  
  #df= data frame
  #trait = name of trait 
  
  name_trait <- quote(trait)

Ctrl<- df %>% 
       dplyr::filter(treatment == "control") %>%  pull(trait)
...
cohensD_per_trait <- function(x, column){
...
try <- cohensD_per_trait(Bole, ldry_weight)
...
trait <- as.character(y) 


Ctrl<- Bole %>% 
       dplyr::filter(treatment == "control") %>%  pull(trait2)
