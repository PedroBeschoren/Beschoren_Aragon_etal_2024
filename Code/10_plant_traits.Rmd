...
```
merge df with qpcr data for later 
...
#open gene expression df 
qpcr.w <- read.csv("./Data/qPCR_output.csv")
qpcr.w[1:6] <- lapply(qpcr.w[1:6], as.factor)
qpcr.w <- qpcr.w %>% 
        dplyr::select(id,Block,species,treatment, MYC2_at_fold_DD, lox2_at_fold_DD)%>% 
        dplyr::rename(myc2 = MYC2_at_fold_DD, lox2=lox2_at_fold_DD)
...
#Arabidopsis
qpcr.Atha <- dplyr::filter(qpcr.w, species=="Arabidopsis_thaliana")
...
qpcr.Bole <- dplyr::filter(qpcr.w, species=="Brassica_oleraceae")
...
START with joining df of qpcr with traits 
