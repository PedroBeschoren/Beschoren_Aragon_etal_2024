...
Now make new df's for qpcr data, has to be long format
```{r}
#pivot longer
qpcr <- qpcr %>% 
...
p.dw.spp <- ggplot(data = traits,
  ...
  scale_y_continuous(expand = c(0, 0), #start graph at y = min and x = min
                     limits = c(0, max(traits$ldry_weight*1.1))) + #y limits
...
#save them in list
p_dw_l <- list(p.dw.Atha, p.dw.Bole)
