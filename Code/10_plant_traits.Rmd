...
p.cohensD <- ggplot(data = d_traits,
                    mapping = aes(x = trait, y = estimate), colour= treatment)+
  geom_bar(mapping = aes(fill = treatment), color="black", size=0.8, stat = "summary", position = position_dodge(width = 0.8))+
  geom_errorbar(aes(ymin=conf.int.lower, ymax=conf.int.upper), width=.4, size=0.8, position = position_dodge(width = 0.8))+
  scale_fill_manual(values = c("#E69F00", "#56B4E9", "#009E73"))+
  labs(y="plant traits measured", x=" ") +
...
p.cohensD <- ggplot(data = d_traits,
  ...
  geom_errorbarh(aes(xmin = conf.int.lower, xmax = conf.int.upper), height = 0.25) +
