...
# Set axis looks
axis_looks <- theme(axis.text.x = element_text(colour = "black", size = 12,
...
colnames(traits) <- gsub(".x\\b", '', colnames(traits))
str(traits)

#make gene_data column factor
traits[c(16)] <- lapply(traits[c(16)], factor)
...
#save in list
lox2 <- list(p.lox2.Atha, p.lox2.Bole) 
names(lox2) <- c("Atha", "Bole")
...
simulateResiduals(lm.lox2.Atha, plot = T) #okay but residuals not normal 
...
#check distribution
continuous_distribution_plot_table(qpcr.Bole$lox2) #not normal, gamma 
hist(qpcr.Bole$lox2)
...
simulateResiduals(lm.lox2.Bole, plot = T) #not ok
...
cld.lox2.Bole <- multcomp::cld(emm.lox2.Bole,
           alpha = 0.05,
           Letters = letters,
           adjust = "tukey")
...
In both Brassica oleracea and Arabidopsis thaliana there's a significant effect of the treatment in the relative expression levels of LOX2, specifically plants treated with the higuest concentration of MeJA had higuer expression of LOX2 compared to controls. 

## 10.9 MYC2 gene expression 
...
#save in list
myc2 <- list(p.myc2.Atha, p.myc2.Bole) 
names(myc2) <- c("Atha", "Bole")
```
...
simulateResiduals(lm.myc2.Bole, plot = T) #not ok
...
Only for Arabidopsis thaliana there's a significant effect of the treatment in the relative expression levels of MYC2, specifically, plants treated with the higuest concentration of MeJA had higuer expression of MYC2 compared to controls. 


## 10.10 Cohen's D comparison 

For tomorrow, plot the standardized mean difference (cohen's D) with confidence intervals for:  
...
cohensD_per_trait <- function(df, column, species){
  ...
  trait_name <- deparse(substitute(column))
  species_name <- deparse(substitute(species))
...
d_MeJA0.1 <- cohen.d(Ctrl, MeJA_0.1) 
d_MeJA1.0 <- cohen.d(Ctrl, MeJA_1) 
d_OS <- cohen.d(Ctrl, OS) 

a <- as.data.frame(t(unlist(d_MeJA0.1))) %>% 
      mutate(treatment="MeJA_0.1")             
...
#changing to factor and numberic 
...
```{r}
#settings for plots 
color_treatments_noControl <- c("#E69F00", "#56B4E9", "#009E73")
...
p.cohensD.Atha <- ggplot(data = subset(d_traits, species == "Arabidopsis_thaliana"), mapping = aes(x = trait, y = estimate,
  ...
  scale_x_discrete(name="plant traits measured", labels=trait_names) +
...
#Brassica
p.cohensD.Bole <- ggplot(data = subset(d_traits, species == "Brassica_oleraceae"), mapping = aes(x = trait, y = estimate,
                    ymin=conf.int.lower, ymax=conf.int.upper,colour= treatment, fill=treatment))+
  geom_linerange(size=5, position=position_dodge(width = 0.5)) +
  geom_hline(aes(yintercept=0), lty=2) +
    geom_hline(aes(yintercept=0.8), lty=2, colour="red") +
...
d_plots_l<- list(p.cohensD.Atha, p.cohensD.Bole) 

d_plots <- marrangeGrob(d_plots_l, nrow=1, ncol=2, top = "Cohen's D")

ggsave(d_plots,
       height = 7, width = 7 * aspect_ratio,
       file="./R output/cohensD_plots.svg")
