...
### 4.9.1b run 100 permanovas on randomly selected OTUs. here we simply collect X otus from the full table
...
dim(rbind(diversity_list$AT_above_rerarefied,
      diversity_list$BO_above_rerarefied))
```
...
taxon_list_output<-taxon_list_output[taxon_list_output != "f__uncultured"]
...
plot_diversity_correlation<-function(full_VS_selected_TaxaLevel){
...
ggplot(data=z,aes(x=Observed_full_dataset,y=Observed_above_expected ))+
  geom_point() +
  geom_smooth(method="lm")+ #values of y are based on the values of x
  labs(subtitle = paste("Spearman rank correlation:",
                        stats::cor(z$Observed_full_dataset,
                                  z$Observed_above_expected,
                                  method = "spearman" ),
                        sep = " "))+
  geom_text_repel(aes(label=z[z$Observed_above_expected>10,1]), data = z[z$Observed_above_expected>10,]))
...
plot_diversity_correlation(full_VS_selected_Class)
plot_diversity_correlation(full_VS_selected_Order)
plot_diversity_correlation(full_VS_selected_Family)


# the loop not working properly! AT and BO data partions are being overwirnt on top of each other
# loop over taxa levels
taxa_levels<-c("Class", "Order", "Family")

above_expected<-list()
full_dataset<-list()
full_VS_selected<-list()
for (i in taxa_levels){
above_expected[i]<-lapply(pslit_neutraly_selected, function (x)
  diversity_per_taxon(ps_object = x,
                      taxa_level = i,
                      data_partition = "above_expected"))

full_dataset[i]<-lapply(ps_list_rarefied[c(2,4)], function (x)
  diversity_per_taxon(ps_object = x,
                      taxa_level = i,
                      data_partition = "full_dataset"))

# join 2 dfs of step 3
full_VS_selected[i]<-mapply(function (x,y)
  left_join(x,
            y,
            by = "Row.names"), # this might change depending on  diversity_per_taxon()
  x = above_expected,
  y = full_dataset,
  SIMPLIFY = FALSE)
}
