...
#### now, R², m and fit class percentage metrics can be extracted as a groupand plotted #### 
...
#get ASV  names in each intersection
ps_venn(uni_4model_BO_above,"MeJA_treatment", weight = TRUE)
venn_asv_list<-ps_venn(uni_4model_BO_above,"MeJA_treatment", plot = FALSE)


```

#### 4.6.6 re-normalize data and plot it

```{r}
##########


#this takes a physeq object and applies the CSS normalization to it
make_CSS_physeq<-function (physeq){
#perform CCS normalization..........................
MRexp_objt<-phyloseq_to_metagenomeSeq(physeq)
...
uni_4model_BO_above_CSS<-physeq

# and now change it's taxa table
otu_table(uni_4model_BO_above_CSS)<-otu_table(CSS_matrix, taxa_are_rows = TRUE)

return(uni_4model_BO_above_CSS)
}

uni_4model_BO_above_CSS<-make_CSS_physeq(uni_4model_BO_above)
uni_4model_BO_asPredicted_CSS<-make_CSS_physeq(uni_4model_BO_asPredicted)
uni_4model_BO_below_CSS<-make_CSS_physeq(uni_4model_BO_below)

test<-list(uni_4model_BO_above_CSS, uni_4model_AT_above_CSS)

names(test)<-c("Brassica_oleraceae.Soil", "Arabdopsis_thaliana.Soil")
### plot above
nmds_uni_4model_BO_above_CSS <- phyloseq::ordinate(uni_4model_BO_above_CSS, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 
...
plot_ordination(physeq = uni_4model_BO_above_CSS, 
                ordination = nmds_uni_4model_BO_above_CSS, 
                type = "sample",
                color= "MeJA_treatment")
### plot  predicted
nmds_uni_4model_BO_asPredicted_CSS <- phyloseq::ordinate(uni_4model_BO_asPredicted_CSS, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 

plot_ordination(physeq = uni_4model_BO_asPredicted_CSS, 
                ordination = nmds_uni_4model_BO_asPredicted_CSS, 
                type = "sample",
                color= "MeJA_treatment")

### plot below
nmds_uni_4model_BO_below_CSS <- phyloseq::ordinate(uni_4model_BO_below_CSS, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 
...
plot_ordination(physeq = uni_4model_BO_below_CSS, 
                ordination = nmds_uni_4model_BO_below_CSS, 
                type = "sample",
                color= "MeJA_treatment")
#
...
BO_soil_predicted_list_CSS<-list(uni_4model_BO_above_CSS,
                                 uni_4model_BO_asPredicted_CSS,
                                 uni_4model_BO_below_CSS)


names(BO_soil_predicted_list_CSS)<-c("uni_4model_BO_above_CSS",
                                     "uni_4model_BO_asPredicted_CSS",
                                     "uni_4model_BO_below_CSS")
...
# 4.7 one model for each AT_soil treatment, using all BO_soil samples as a pool
...
#### 4.7.6 re-normalize data and plot it
...
#make a new phyloseq object...
uni_4model_AT_above_CSS<-physeq

# and now change it's taxa table
otu_table(uni_4model_AT_above_CSS)<-otu_table(CSS_matrix, taxa_are_rows = TRUE)

return(uni_4model_AT_above_CSS)
}

uni_4model_AT_above_CSS<-make_CSS_physeq(uni_4model_AT_above)
uni_4model_AT_asPredicted_CSS<-make_CSS_physeq(uni_4model_AT_asPredicted)
uni_4model_AT_below_CSS<-make_CSS_physeq(uni_4model_AT_below)


### plot above
nmds_uni_4model_AT_above_CSS <- phyloseq::ordinate(uni_4model_AT_above_CSS, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 
...
plot_ordination(physeq = uni_4model_AT_above_CSS, 
                ordination = nmds_uni_4model_AT_above_CSS, 
                type = "sample",
                color= "MeJA_treatment")
### plot  predicted
nmds_uni_4model_AT_asPredicted_CSS <- phyloseq::ordinate(uni_4model_AT_asPredicted_CSS, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 

plot_ordination(physeq = uni_4model_AT_asPredicted_CSS, 
                ordination = nmds_uni_4model_AT_asPredicted_CSS, 
                type = "sample",
                color= "MeJA_treatment")

### plot below
nmds_uni_4model_AT_below_CSS <- phyloseq::ordinate(uni_4model_AT_below_CSS, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 
...
plot_ordination(physeq = uni_4model_AT_below_CSS, 
                ordination = nmds_uni_4model_AT_below_CSS, 
                type = "sample",
                color= "MeJA_treatment")
#
```
...
AT_soil_predicted_list_CSS<-list(uni_4model_AT_above_CSS,
                                 uni_4model_AT_asPredicted_CSS,
                                 uni_4model_AT_below_CSS)


names(AT_soil_predicted_list_CSS)<-c("uni_4model_AT_above_CSS",
                                     "uni_4model_AT_asPredicted_CSS",
                                     "uni_4model_AT_below_CSS")

#beta dispersion
dip_result4<-beta_disp_plotAndTest(AT_soil_predicted_list_CSS, "MeJA_treatment")

#Permanova
permanova_with_blocks(AT_soil_predicted_list_CSS, "MeJA_treatment + Block") 

#pairwise1
lapply(AT_soil_predicted_list_CSS, function (x)
  adonis.pair(dist.mat= phyloseq::distance(otu_table(x), method="bray"),
              Factor= as.factor(as(phyloseq::sample_data(x),"data.frame")$MeJA_treatment)))


#pairwise2
parwise_above<-pairwise.adonis2(phyloseq::distance(otu_table(AT_soil_predicted_list_CSS[[1]]), method="bray") ~ MeJA_treatment+Block, data = as(phyloseq::sample_data(AT_soil_predicted_list_CSS[[1]]),"data.frame"), strata="Block")

parwise_aspredicted<-pairwise.adonis2(phyloseq::distance(otu_table(AT_soil_predicted_list_CSS[[2]]), method="bray") ~ MeJA_treatment+Block, data = as(phyloseq::sample_data(AT_soil_predicted_list_CSS[[2]]),"data.frame"), strata="Block")

parwise_below<-pairwise.adonis2(phyloseq::distance(otu_table(AT_soil_predicted_list_CSS[[3]]), method="bray") ~ MeJA_treatment+Block, data = as(phyloseq::sample_data(AT_soil_predicted_list_CSS[[3]]),"data.frame"), strata="Block")
...
hundred_permanovas_on_random_otu<-  function (CSS_fraction,uni_4model) {
  ...
  # thes econd argument is the phyloseq object contaning only the OTUs classified as above expected
  # The output is a list of 100 permanovas
  
# collect a list of random OTUS, 100 times
set.seed(92342347)

# uni_4model$MeJA_0.1) won't work aocrss the function
rand_otu_control<-replicate(100, sample(taxa_names(subset_samples(CSS_fraction, MeJA_treatment = "Control")), nrow(tax_table(uni_4model[[1]])), replace=FALSE)) #performs randomization oepration 100 times

rand_otu_MeJA_0.1<-replicate(100, sample(taxa_names(subset_samples(CSS_fraction, MeJA_treatment = "MeJA_0.1")), nrow(tax_table(uni_4model[[2]])), replace=FALSE)) #performs randomization oepration 100 times
...
### 4.9.1b run 100 permanovas on randomly selected OTUs
...
# collect a list of random OTUS, 100 times
set.seed(92342347)
rand_otu<-replicate(100, sample(taxa_names(CSS_fraction), nrow(tax_table(uni_4model)), replace=FALSE)) #performs randomization oepration 100 times. the number of OTUs randomly sampled will be equal the the numebr of OTUs in the above-selected phyloseq objects

#turn the n_rows x 100df into a list of 100 vectors
Rand_otus_100_lists<-list() #make empty list
for(i in 1:ncol(rand_otu)) {             # Using for-loop to add columns to list
  Rand_otus_100_lists[[i]] <- rand_otu[ , i]
}
...
permanovas_BO_100<-hundred_permanovas_on_random_otu(ps_list_CSS$Brassica_oleraceae.Soil, uni_4model_BO_above)
permanovas_AT_100<-hundred_permanovas_on_random_otu(ps_list_CSS$Arabidopsis_thaliana.Soil, uni_4model_AT_above)
...
# quick plot bar with relative abudances, just to show how outdated it is
 ...
 silly_plot$Phylum <- as.character(data$Phylum)
 silly_plot$Class <- as.character(data$Class)
...
neutral_heat<-parse_phyloseq(neutral_heat_tree_BO)

#get abundance per taxon
neutral_heat$data$tax_abund<-calc_taxon_abund(obj = neutral_heat, 
                                      data = "otu_table",
                                      cols = neutral_heat$data$sample_data$sample_id)
#get occuence of per sample type
neutral_heat$data$tax_occ <- calc_n_samples(obj = neutral_heat, 
                                    data = "tax_abund", 
                                    groups = neutral_heat$data$sample_data$MeJA_treatment, 
                                    cols = neutral_heat$data$sample_data$sample_id)
...
neutral_heat$data$diff_table <- compare_groups(obj = neutral_heat,
                                      dataset = "tax_abund",
                                      cols = neutral_heat$data$sample_data$sample_id, # What columns of sample data to use
                                      groups = neutral_heat$data$sample_data$MeJA_treatment) # What category each sample is assigned to

# set differental log ratio to 0 based on adjusted p values
neutral_heat$data$diff_table$log2_median_ratio[neutral_heat$data$diff_table$wilcox_p_value > 0.05] <- 0
...
neutral_heat$data$diff_table$treatment_1
tail(neutral_heat$data$otu_table)


#plot matrix tree
set.seed(1)
heat_tree_matrix(neutral_heat,
                 data = "diff_table",
                 ...
                 output_file = "differential_heat_tree_BO.pdf") # Saves the plot as a pdf file
...
##################### list of matrixes of ehatmaps


#remove unecessary taxonomic indo (dada2id, "S__" and" above_selected)
tax_table(neutral_heat_tree)<-tax_table(neutral_heat_tree)[,1:6]



# transform from phyloseq to  taxmap object
test<-parse_phyloseq(neutral_heat_tree)
