...
list_100_phyloseqs_control <- lapply(Rand_otus_control_lists, function (x)
  prune_taxa(x, subset_samples(CSS_fraction, MeJA_treatment == "control"))) 
list_100_phyloseqs_meja01 <- lapply(Rand_otus_meja01_lists, function (x)
  prune_taxa(x, subset_samples(CSS_fraction, MeJA_treatment == "MeJA_0.1"))) 
list_100_phyloseqs_meja10 <- lapply(Rand_otus_meja10_lists, function (x)
  prune_taxa(x, subset_samples(CSS_fraction, MeJA_treatment == "MeJA_1.0"))) 
list_100_phyloseqs_os <- lapply(Rand_otus_oral_lists, function (x)
  prune_taxa(x, subset_samples(CSS_fraction, MeJA_treatment == "oral_secretion"))) 

#merge ps objects
merged_100_ps<-mapply (function(z,x,y,w)
  merge_phyloseq(z,x,y,w),
x = list_100_phyloseqs_control,
z = list_100_phyloseqs_meja01,
y = list_100_phyloseqs_meja10,
w = list_100_phyloseqs_os,
SIMPLIFY = FALSE)

#run 100 permanovas
set.seed(9247)
permanovas_100<-permanova_with_blocks(merged_100_ps, "MeJA_treatment + Block")
...
histogram_artefact_p<-ggplot(bind_rows(p_values_MeJA_vector_AT,
  ...
  geom_vline(xintercept = log(permanova_AT_above[[5]][1]), #p values of BO in the above-expected model
             linetype="dashed",
             size=1.5,
             colour = "#00BFC4")+ 
  ...
  labs(title="100 PERMANOVAS, dashed neutral, solid full, p values",
       x="Log(p values)", 
       y = "Count")+
  theme_classic()+
...
histogram_artefact_R2<-ggplot(bind_rows(R2_MeJA_vector_AT,
  ...
  labs(title="100 PERMANOVAS, dashed neutral, solid full, R2 values",
       x="R2 values", 
       y = "Count")+
    theme_classic()+
  theme(legend.position = "top")
...
histogram_artefact_F<-ggplot(bind_rows(F_value_vector_AT,
  ...
  geom_vline(xintercept = full_feature_permanova[[4]][[4]][1], #F2 values of BO in the full feature permanova
             ...
             size=1.5,
             colour = "#00BFC4")+ #  color code for the standard second facotr of a ggplot (teal)
    labs(title="100 PERMANOVAS, dashed neutral, solid full, F values",
       x="F values", 
       y = "Count")+
  theme_classic()+
  theme(legend.position = "top")
