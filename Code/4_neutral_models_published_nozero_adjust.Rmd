...
Neutral_plots_list_BO_soil_g <- ggarrange(Neutral_plots_list_BO_soil$control,
...
#save plots
ggsave(Neutral_plots_list_BO_soil_g,
       height = 9, width = 12,
       file="./R output/Figures_for_publication/NeutralModels_BO.svg")
...
#### 4.6.5 select only one of the fit classes in BO_soil, then merge the 4 treatment split
...
BO_soil_predicted_list<-subset_ps_above_neutral_below(ps_l_predictions_in_taxtable = ps_list_neutral_BO_soil,
...
compare the metrics of the chunk above to values of a neutral model where the 4 treatmensts are not split (a single model for all 24 smaples in BO_soil). the difference is that we are no longer looking at ASVs that were selected by treatment, but instead we look at ASVs that were selected in BO_soil
...
####  4.6.7 check plots of each subcommunity set

```{r}
lapply(BO_soil_predicted_list, function(x){
set.seed(101)
ordination_NMDS <- phyloseq::ordinate(x, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 
...
Venn_BO<-venn_neutral(ps_l_predictions_in_taxtable = ps_list_neutral_BO_soil)+
  ggtitle(label = "Above Neutral ASV overlap, B. oleraceae")
...
#### 4.7.5 select only one of the fit classes in AT_soil, then merge the 4 treatment split
...
#execute the custom function above on BO soil samples
AT_soil_predicted_list<-subset_ps_above_neutral_below(ps_l_predictions_in_taxtable = ps_list_neutral_AT_soil,
...
#### 4.7.7 check plots of each subcommunity set
...
#run the custom function
Venn_AT<-venn_neutral(ps_l_predictions_in_taxtable = ps_list_neutral_AT_soil)+
...
ggsave(export_venn,
       height = 5, width = 10,
       file="./R output/Figures_for_publication/NeutralModel_Veen.svg")
...
plot_nmds_check_BO_soil <- plot_ordination(physeq = check_BO_soil, 
                ordination = nmds_check_BO_soil, 
                type = "sample",
                color= "MeJA_treatment")


plot_nmds_check_BO_soil_ed <- plot_nmds_check_BO_soil + 
  geom_point(size=5, alpha=0.8)+
  axis_looks
...
plot_nmds_check_AT_soil <- plot_ordination(physeq = check_AT_soil, 
                ordination = nmds_check_AT_soil, 
                type = "sample",
                color= "MeJA_treatment")

plot_nmds_check_AT_soil_ed <- plot_nmds_check_AT_soil + 
  geom_point(size=5, alpha=0.8)+
  axis_looks

#Arrange plots 
plot_nmds_check_AT_BO_soil_ed <- ggarrange(plot_nmds_check_AT_soil_ed, 
                                           plot_nmds_check_BO_soil_ed,
                                           ncol = 2)
#save plots
ggsave(plot_nmds_check_AT_BO_soil_ed,
       height = 5, width = 15,
       file="./R output/Figures_for_publication/NeutralModels_NMDS_soil_AT_BO.svg")

```

### 4.8.2 plot BO with AT, above expected only

First, one plot per species 
...
#CSS normalization
BO_above_CSS <- make_CSS_physeq(uni_4model_BO_above)
AT_above_CSS <- make_CSS_physeq(uni_4model_AT_above)

### plot above BO
set.seed(123)
nmds_BO_above_CSS <- phyloseq::ordinate(BO_above_CSS, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 

plot_nmds_BO_above_CSS <- plot_ordination(physeq = BO_above_CSS, 
                ordination = nmds_BO_above_CSS, 
                type = "sample",
                color= "MeJA_treatment")

plot_nmds_BO_above_CSS_ed <- plot_nmds_BO_above_CSS + 
  geom_point(size=5, alpha=0.8)+
  axis_looks

### plot above AT
set.seed(123)
nmds_AT_above_CSS <- phyloseq::ordinate(AT_above_CSS, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 

plot_nmds_AT_above_CSS <- plot_ordination(physeq = AT_above_CSS, 
                ordination = nmds_AT_above_CSS, 
                type = "sample",
                color= "MeJA_treatment")

plot_nmds_AT_above_CSS_ed <- plot_nmds_AT_above_CSS + 
  geom_point(size=5, alpha=0.8)+
  axis_looks

#Arrange plots 
plot_nmds_above_AT_BO_soil_ed <- ggarrange(plot_nmds_AT_above_CSS_ed, 
                                           plot_nmds_BO_above_CSS_ed,
                                           ncol = 2)
#save plots
ggsave(plot_nmds_above_AT_BO_soil_ed,
       height = 5, width = 15,
       file="./R output/Figures_for_publication/NeutralModels_NMDS_above_AT_BO.svg")


```


Then, both species in a single plot
```{r}
############## above expected onlu
...
rand_otu_control<-replicate(100, sample(taxa_names(subset_samples(CSS_fraction, MeJA_treatment == "control")), nrow(tax_table(ps_4_model_l[[1]])), replace=FALSE)) #performs randomization operation 100 times

rand_otu_MeJA_0.1<-replicate(100, sample(taxa_names(subset_samples(CSS_fraction, MeJA_treatment == "MeJA_0.1")), nrow(tax_table(ps_4_model_l[[2]])), replace=FALSE)) #performs randomization operation 100 times

rand_otu_MeJA_1.0<-replicate(100, sample(taxa_names(subset_samples(CSS_fraction, MeJA_treatment == "MeJA_1.0")), nrow(tax_table(ps_4_model_l[[3]])), replace=FALSE)) 

rand_otu_oral_scretion<-replicate(100, sample(taxa_names(subset_samples(CSS_fraction, MeJA_treatment == "oral_secretion")), nrow(tax_table(ps_4_model_l[[4]])), replace=FALSE)) 
...
permanovas_BO_100<-hundred_permanovas_on_random_otu(CSS_fraction =  ps_list_CSS$Brassica_oleraceae.Soil, 
                                                    ps_4_model_l = BO_4_model_above)
permanovas_AT_100<-hundred_permanovas_on_random_otu(CSS_fraction = ps_list_CSS$Arabidopsis_thaliana.Soil, 
                                                    ps_4_model_l = AT_4_model_above)
...
permanova_AT_above<-permanova_with_blocks(BO_soil_predicted_list, "MeJA_treatment + Block")$Brassica_oleraceae.Soil.above
permanova_BO_above<-permanova_with_blocks(AT_soil_predicted_list, "MeJA_treatment + Block")$Arabidopsis_thaliana.Soil.above
...
histogram_artefact_p<-ggplot(bind_rows(p_values_MeJA_vector_AT,
  ...
  geom_vline(xintercept = log(permanova_BO_above[[5]][1]), #p values of BO in the above-expected model
             ...
             size=1.5,
             colour = "#00BFC4")+ 
  geom_vline(xintercept = log(full_feature_permanova[[2]][[5]][1]), #p values of AT in the full feature permanova
             linetype="dotted",
             size=1.5,
             colour = "#F8766D")+ #  color code for the standard second facotr of a ggplot (teal)
  geom_vline(xintercept = log(full_feature_permanova[[4]][[5]][1]), #p values of BO in the full feature permanova
             linetype="dotted",
             size=1.5,
...
histogram_artefact_R2<-ggplot(bind_rows(R2_MeJA_vector_AT,
  ...
  geom_vline(xintercept = permanova_BO_above[[3]][1], #R2 values of BO, in the above-expected model
             ...
             size=1.5,
             colour = "#00BFC4")+ #  color code for the standard second facotr of a ggplot (teal)
  geom_vline(xintercept = full_feature_permanova[[2]][[3]][1], #R2 values of AT in the full feature permanova
             linetype="dotted",
             size=1.5,
             colour = "#F8766D")+ #  color code for the standard second facotr of a ggplot (teal)
  geom_vline(xintercept = full_feature_permanova[[4]][[3]][1], #R2 values of BO in the full feature permanova
             linetype="dotted",
             size=1.5,
             colour = "#00BFC4")+ #  color code for the standard second facotr of a ggplot (teal)
...
histogram_artefact_F<-ggplot(bind_rows(F_value_vector_AT,
  ...
  geom_vline(xintercept = permanova_BO_above[[4]][1], #F2 values of BO, in the above-expected model
             ...
             size=1.5,
             colour = "#00BFC4")+
  geom_vline(xintercept = full_feature_permanova[[2]][[4]][1], #F2 values of AT in the full feature permanova
             linetype="dotted",
             size=1.5,
             colour = "#F8766D")+ #  color code for the standard second facotr of a ggplot (teal)
  geom_vline(xintercept = full_feature_permanova[[4]][[4]][1], #F2 values of BO in the full feature permanova
             linetype="dotted",
             size=1.5,
             colour = "#00BFC4")+ #  color code for the standard second facotr of a ggplot (teal)
...
neutral_heat_tree_AT<-subset_ps_above_neutral_below(ps_l_predictions_in_taxtable = ps_list_neutral_AT_soil,
                                                      ps_CSS_partition = ps_list_rarefied$Arabidopsis_thaliana.Soil)[[1]]


#new ps object, based on rarefied data
neutral_heat_tree_BO<-lapply(ps_list_neutral_BO_soil, function (x) subset_taxa(x, fit_class == "Above prediction" ))
...
# The layout algorithm that initializes node locations
                 output_file = "./R output/differential_heat_tree_BO_rhizo.pdf") # Saves the plot as a pdf file
...
#new ps object, based on rarefied data
neutral_heat_tree_AT<-lapply(ps_list_neutral_AT_soil, function (x) subset_taxa(x, fit_class == "Above prediction" ))
...
heat_tree_matrix(test,
                 ...
                 output_file = "./R output/differential_heat_tree_AT_rhizo.pdf") # Saves the plot as a pdf file
...
#new ps object, based on rarefied data
ps_AT_alpha_cor_above<-subset_ps_above_neutral_below(ps_l_predictions_in_taxtable = ps_list_neutral_BO_soil,
                                                      ps_CSS_partition = ps_list_rarefied$Brassica_oleraceae.Soil)[[1]]

ps_BO_alpha_cor_above<-subset_ps_above_neutral_below(ps_l_predictions_in_taxtable = ps_list_neutral_AT_soil,
                                                      ps_CSS_partition = ps_list_rarefied$Arabidopsis_thaliana.Soil)[[1]]
...
fivecorr_BO<-hundred_correlations_on_random_otu(CSS_fraction =  ps_list_rarefied$Brassica_oleraceae.Soil, 
                                              ps_neutral_above = BO_soil_predicted_list$Brassica_oleraceae.Soil.above)
