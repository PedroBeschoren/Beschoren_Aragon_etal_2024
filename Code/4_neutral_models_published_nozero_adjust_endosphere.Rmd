...
neutral_list<- lapply(ps_list_rarefied, function(x)
  tyRa::fit_sncm(spp = t(otu_table(x)), 
                 pool = NULL, # Pool acts as a source of microbes, such as full bulk Root diversity. if NULL, you use the input taxa talbe as the pool for every individual sample inside your phyloseq object
...
set.seed(5235)
  ...
  adonis.pair(dist.mat= phyloseq::distance(otu_table(AT_Root_predicted_list$Arabidopsis_thaliana.Root.above), method="bray"),
              Factor= as.factor(as(phyloseq::sample_data(AT_Root_predicted_list$Arabidopsis_thaliana.Root.above),"data.frame")$MeJA_treatment))
...
####!!! endosphre samples missing funcion and objects:
...
```{r}
# original objects: 
ps_list_rarefied$Brassica_oleraceae.Root
ps_list_rarefied$Arabidopsis_thaliana.Root

#re-merged BO
check_BO_Root<-remove_tree_and_merge(ps_list_neutral_BO_Root)

sample_sums(check_BO_Root)

set.seed(101)
nmds_check_BO_Root <- phyloseq::ordinate(check_BO_Root, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 
...
plot_nmds_check_BO_Root <- plot_ordination(physeq = check_BO_Root, 
                ordination = nmds_check_BO_Root, 
                type = "sample",
                color= "MeJA_treatment")


plot_nmds_check_BO_Root_ed <- plot_nmds_check_BO_Root + 
  geom_point(size=5, alpha=0.8)+
  axis_looks


#re-merged AT
check_AT_Root<-remove_tree_and_merge(ps_list_neutral_AT_Root)


nmds_check_AT_Root <- phyloseq::ordinate(check_AT_Root, 
                             method ="NMDS", 
                             distance="bray", 
                             try=200,
                             autotransform=TRUE) 
...
plot_nmds_check_AT_Root <- plot_ordination(physeq = check_AT_Root, 
                ordination = nmds_check_AT_Root, 
                type = "sample",
                color= "MeJA_treatment")

plot_nmds_check_AT_Root_ed <- plot_nmds_check_AT_Root + 
  geom_point(size=5, alpha=0.8)+
  axis_looks

#Arrange plots 
plot_nmds_check_AT_BO_Root_ed <- ggarrange(plot_nmds_check_AT_Root_ed, 
                                           plot_nmds_check_BO_Root_ed,
                                           ncol = 2)
#save plots
#ggsave(plot_nmds_check_AT_BO_Root_ed,
 #      height = 5, width = 15,
  #     file="./R output/Figures_for_publication/NeutralModels_NMDS_Root_AT_BOendosphere.svg")
...
#Arrange plots 
plot_nmds_above_AT_BO_Root_ed <- ggarrange(plot_nmds_AT_above_CSS_ed, 
                                           plot_nmds_BO_above_CSS_ed,
                                           ncol = 2)
#save plots
ggsave(plot_nmds_above_AT_BO_Root_ed,
       height = 5, width = 15,
       file="./R output/Figures_for_publication/NeutralModels_NMDS_above_AT_BOendosphere.svg")
...
neutral_heat_tree_AT<-subset_ps_above_neutral_below(ps_l_predictions_in_taxtable = ps_list_neutral_AT_Root,
