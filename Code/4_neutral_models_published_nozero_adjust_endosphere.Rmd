...
set.seed(5235)
permanova_AT_above<-permanova_with_blocks(AT_Root_predicted_list, "MeJA_treatment + Block")$Arabidopsis_thaliana.Root.above
set.seed(5235)
permanova_BO_above<-permanova_with_blocks(BO_Root_predicted_list, "MeJA_treatment + Block")$Brassica_oleraceae.Root.above
...
# define a vector with interesting taxa we want to plot
interesting_taxa<-c("Actinobacteria", "Alphaproteobacteria", "Bacilli", "Bacteroidia", "Gammaproteobacteria", "Chloroflexi", "Firmicutes", "Acidobacteriota")
...
ggplot(plot_l_obs_Family$uni_4model_AT_above) + ggplot(plot_l_obs_Family$uni_4model_BO_above)

plot_l_obs_Family$uni_4model_AT_above + plot_l_obs_Family$uni_4model_BO_above
...
full_VS_selected_Family$uni_4model_AT_above




ggplot(data=full_VS_selected_Family$uni_4model_AT_above[full_VS_selected_Family$uni_4model_AT_above$Simpson_above_expected>0,], #only includes shannon >0
       aes(x=Simpson_full_dataset,y=Simpson_above_expected ))+
  geom_point() +
   theme_bw()+
  theme(axis.title=element_text(size=11, face = "bold"))+
  ...
  ylab(label = "Simpson diversity in above-expected subset")+
  geom_smooth(method="loess")+ #values of y are based on the values of x
    geom_text_repel(aes(label=full_VS_selected_Family$uni_4model_AT_above[(full_VS_selected_Family$uni_4model_AT_above[,1] %in% intereting_taxa),][,1]), 
                    data = full_VS_selected_Family$uni_4model_AT_above[(full_VS_selected_Family$uni_4model_AT_above[,1] %in% intereting_taxa),], size = 3)

interesting_taxa<-c("Burkholderiaceae", "Comamonadaceae", "Rhizobiaceae", "asdas")

full_VS_selected_Family$uni_4model_AT_above[(full_VS_selected_Family$uni_4model_AT_above[,1] %in% intereting_taxa),][,1]

z[(z[,1] %in% intereting_taxa),][,1]

label=full_VS_selected_Family$uni_4model_AT_above[full_VS_selected_Family$uni_4model_AT_above$Simpson_above_expected >0,1]
...
ggplot(data=z,aes(x=Observed_full_dataset,y=Observed_above_expected ))+
  ...
  ylab(label = "Number of taxa in above-expected subset")+
  geom_smooth(method="loess")+ #values of y are based on the values of x
  geom_text_repel(aes(label=z[(z[,1] %in% interesting_taxa),][,1]), data = z[(z[,1] %in% interesting_taxa),], size = 3))
