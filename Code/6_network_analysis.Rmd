...
library(ggfortify)
autoplot(list_network_metrics.pca,
         label = TRUE,
         loadings = TRUE, 
         loadings.colour = 'red',
         loadings.label = TRUE, 
...
ps_list_network_CSS$Arabidopsis_thaliana.Root@sam_data
...
plots_eigengenes<-plot_module_and_metadata_correlations_output(module_and_metadata_correlations)

ggarrange(plotlist = plots_eigengenes,
          common.legend = TRUE,
          labels = c("A", "B", "C", "D"))
...
ggplot(data = filter(df_nodes_taxCount, module<99, n>2),
        aes(x = as.factor(Family), y = log(Degree) )  )+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  geom_boxplot(aes())
})
...
#subset to important taxa
imp_network_data<-lapply(complete_node_metrics_withFitClass, function(x)
filter(x, Zi>=2.5 | Pi >=0.62 | keystone_taxa =="Keystone"))



# change null value of kystone
imp_network_data<-lapply(imp_network_data, function(x){
x$keystone_taxa[is.na(x$keystone_taxa)]<-""
return(x)
})
...
# set new varaible with all definitions
imp_network_data<-lapply(imp_network_data, function(x)
  mutate(x, network_tag = ifelse(test = keystone_taxa =="Keystone", 
                                 yes = "keystone", 
                                 no = ifelse(test = Zi>=2.5, 
                                             yes ="module_hub",
                                             no = ifelse(test = Pi>=0.62, 
                                                         yes = "module_connector",
                                                         no = "peripheral")))))

#check most common classes, families adn orders
imp_network_data_unlisted<-bind_rows(imp_network_data, .id = "sub_community")
table(imp_network_data_unlisted$Class)%>%sort
table(imp_network_data_unlisted$Order)%>%sort
table(imp_network_data_unlisted$Family)%>%sort
table(imp_network_data_unlisted$Genus)%>%sort

# make the table longer
ggplot_input<-lapply(imp_network_data, function(x)
  pivot_longer(data =x,
               cols =  c("freq_in_control", "freq_in_MeJA_0.1", "freq_in_MeJA_1.0","freq_in_oral_secretion" ),
               values_to = "frequency",
               names_to = "Stress"))



# unlist so we can make a facet wrap plot
ggplot_input_unlisted<-bind_rows(ggplot_input, .id = "sub_community")


ggplot_input_unlisted$Row.names<-as.character(ggplot_input_unlisted$Row.names)

#plot
ggplot(ggplot_input_unlisted, aes(x = name, y = log(frequency), fill = network_tag))+
  geom_boxplot()+
  facet_wrap(~sub_community)

ggplot(ggplot_input_unlisted, aes(x = name, y = log(frequency), fill = network_tag))+
  geom_boxplot()

ggplot(ggplot_input, aes(x = name, y = log(frequency), fill = sub_community))+
  geom_violin()
...
#log scale
ggplot_input_unlisted$log_frequency<-log(ggplot_input_unlisted$frequency)

ggballoonplot(data=ggplot_input_unlisted,
               x = "network_tag",
               y = "name",
              facet.by = c("sub_community"),
              size = "Degree",
              fill = "log_frequency")+
   scale_fill_viridis()+
 scale_color_viridis()
...
Anova(lm((frequency) ~  network_tag *sub_community* Stress, data = ggplot_input_unlisted, contrasts=list(network_tag=contr.sum, sub_community=contr.sum)), type = "2") 
HSD.test(aovTukey, "tx", group=TRUE, console=TRUE)#post-hoc


# data partitions make a huge differe,ce, let's separate them
lapply(ggplot_input, function(x)
leveneTest(log(frequency) ~ network_tag* Stress, data = x) )

lapply(ggplot_input, function(x)
Anova(lm((log(frequency)) ~  network_tag * Stress, data = x, contrasts=list(network_tag=contr.sum, Stress=contr.sum)), type = "2") )
...
# check which is the most highly connected family
lapply(complete_node_metrics_withFitClass, function(x)
ggplot(x,
       aes(x = Family, y = Degree))+
  geom_boxplot()+
...
# check ASVs that were relevant in entwork
net_asvs<-lapply(complete_node_metrics_withFitClass, function(x)
as.character(filter(.data = x,
       Zi > 2.5 |
         Pi >0.62 |
         keystone_taxa == "Keystone")[,1]))


# plot abundances of teh ASVs selected aove
mapply(function(z,y){
  
  ps<-prune_taxa(taxa = z, x = y)
  
  ps<-subset_taxa(physeq = ps, Family =="f__Comamonadaceae" | Family =="f__Streptomycetaceae")
  
  plot_bar(ps, x = "MeJA_treatment", facet_grid = "Family")
  },
  z = net_asvs,
  y = ps_list_rarefied,
  SIMPLIFY = FALSE
