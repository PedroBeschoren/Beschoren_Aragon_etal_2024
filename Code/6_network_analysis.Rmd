...
list_random_VS_real<-map(listed_igraph,Real_VS_random_networks)
...
write.csv(list_network_metrics, "./R output/metrics.csv")
...
## BUG ### 6.7 - Correlate netowrk module structure to plant metadata ####
...
# before we correlated ASV abundance to metadata, we must normalize the original phyloseq object  - speic.easi had it's own nomalization
# I am not sure on how CSS normalization would affect this, so I will use rarefaction to be conservative
set.seed(100)
ps_list_network_rarefied<-lapply(ps_list_network, function(x)
  ...
  )# we don't seem to lose any OTU with this process - this is expected as rare OTUs had already been removed
...
module_and_metadata_correlations<-mapply(function (x,y)
                                         adjustInput_run_eigen_correlation_adjustOutput(igraph_obj = x,
                                                                                        phyloseq_ob = y),
                                         x = listed_igraph,
                                         y= ps_list_network_rarefied, #as noted above, this has been rarefied after the cut-off point to be in the network input
                                         SIMPLIFY = FALSE
                                         )
