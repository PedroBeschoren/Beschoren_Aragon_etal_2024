...
##############################################################################################################
#to a single object, returning dataframe
tax_table(ps_list_rarefied$Arabidopsis_thaliana.Root)%>%
  as.data.frame()%>%
mutate(random_forest_relevance = 
         if_else(taxa_names(ps_list_rarefied$Arabidopsis_thaliana.Root)%in%boruta_ASV_list$Arabidopsis_thaliana.Root == TRUE,"important", "uninportant"))
...
# see taxonomies of RF asvs
important_rf_l<-lapply(rf_added_tax, function(x)
  tax_table(filter(x, random_forest_relevance == "important")))
...
# quick plot bar with relative abudances, for a quick overview of dity data
 silly_plot<-psmelt(transform_sample_counts(rf_important_ps_l$Arabidopsis_thaliana.Soil, function(x) x / sum(x) ))
 silly_plot$Phylum <- as.character(data$Phylum)
 silly_plot$Class <- as.character(data$Class)

# phyla level
ggplot(data=silly_plot, aes(x=MeJA_treatment, y =Abundance, fill=Order))+
geom_bar(aes(), stat="identity", position="stack")+
scale_fill_hue()+
theme_bw()


ggplot(data=silly_plot, aes(x=OTU, y =Abundance, fill=MeJA_treatment))+
geom_bar(aes(), stat="identity", position="stack")+
scale_fill_hue()+
theme_bw()
...
psmelt(transform_sample_counts(merge_phyloseq(rf_important_ps_l[[1]],
               rf_important_ps_l[[2]],
               rf_important_ps_l[[3]],
               rf_important_ps_l[[4]])
