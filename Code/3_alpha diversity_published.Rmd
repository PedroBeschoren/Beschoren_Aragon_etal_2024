...
Here we check both observed diversity and shanon diversity indexes
```{r}
...
leveneTest((Observed) ~ Plant_species* MeJA_treatment * Sample_type, data = total_diversity) 
...
plot_alpha_BySample <- ggplot(total_diversity, aes(x =MeJA_treatment, y = Shannon, fill=Plant_species))+
  geom_boxplot()+
  #geom_jitter(color="black", size=0.7, alpha=0.5, width = 0.2)+
  theme_bw()+
...
ggplot(total_diversity, aes(x =MeJA_treatment, y = Shannon))+
  geom_boxplot()+
  theme_bw()



# sample type (root/soil) has a huge effect on diversity (which we knew before we started the experiment), which depends on species (and we could see this on the first plot). let's try to further separate this, slicing out dataset into lists
...
lapply(diversity_list, function (x)
  leveneTest((Observed) ~ MeJA_treatment, data = x))
...
# check levenes homogeniety over the list
lapply(diversity_list, function (x)
  leveneTest((Observed) ~ MeJA_treatment*Plant_species, data = x))
